...
public class AppUpdateUtils {
    ...
    public static boolean appIsDownloaded(Context context, UpdateAppBean updateAppBean) {
        ...
        if (context != null) {
            ...
            packageInfo = pm.getPackageArchiveInfo(appFile.getAbsolutePath(), 0);
        }
        return appFile.exists() && packageInfo != null;
    }
    ...
    public static boolean installApp(Activity activity, File appFile) {
        try {
            ...
            if (activity != null & activity.getPackageManager().queryIntentActivities(intent, 0).size() > 0) {
