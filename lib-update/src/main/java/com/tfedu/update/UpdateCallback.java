    ...
    protected UpdateAppBean parseJson(String json) {
        ...
        try {
           ...
           // updateAppBean.setUpdate(jsonObject.optString("update"))
                    //存放json，方便自定义解析
                 //   .setOriginRes(json)
            updateAppBean.setNewVersion(jsonObject.optString("latestVersion"))
                    .setApkFileUrl(jsonObject.optString("apkDownloadUrl"))
                 //   .setTargetSize(jsonObject.optString("target_size"))
                 //   .setUpdateLog(jsonObject.optString("update_log"))
                    .setConstraint(jsonObject.optString("mustBeUpdate"));
