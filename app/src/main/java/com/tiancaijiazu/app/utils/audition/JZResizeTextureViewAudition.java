...
public class JZResizeTextureViewAudition extends TextureView {
    protected static final String TAG = "JZResizeTextureView";

    public int currentVideoWidth = 0;
    public int currentVideoHeight = 0;

    public JZResizeTextureViewAudition(Context context) {
        super(context);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }

    public JZResizeTextureViewAudition(Context context, AttributeSet attrs) {
        super(context, attrs);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE == JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE == JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                /**原图**/
                height = videoHeight;
                width = videoWidth;
            } else if (JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE == JZVideoPlayerAudition.VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
