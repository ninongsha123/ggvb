...
public class JZMediaManagerAudition implements TextureView.SurfaceTextureListener {

    public static final String TAG = "JiaoZiVideoPlayer";
    public static final int HANDLER_PREPARE = 0;
    public static final int HANDLER_RELEASE = 2;

    public static JZResizeTextureViewAudition textureView;
    public static SurfaceTexture savedSurfaceTexture;
    public static Surface surface;
    public static JZMediaManagerAudition jzMediaManager;
    public int positionInList = -1;
    public JZMediaInterfaceAudition jzMediaInterface;
    ...
    public JZMediaManagerAudition() {
        mMediaHandlerThread = new HandlerThread(TAG);
        mMediaHandlerThread.start();
        mMediaHandler = new MediaHandler(mMediaHandlerThread.getLooper());
        mainThreadHandler = new Handler();
        if (jzMediaInterface == null)
            jzMediaInterface = new JZMediaSystemAudition();
    }

    public static JZMediaManagerAudition instance() {
        if (jzMediaManager == null) {
            jzMediaManager = new JZMediaManagerAudition();
        }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int i, int i1) {
        Log.i(TAG, "onSurfaceTextureAvailable [" + JZVideoPlayerManagerAudition.getCurrentJzvd().hashCode() + "] ");
